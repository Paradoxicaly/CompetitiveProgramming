...
void solve(){
    ...
    for(int i=1;i<=n;i++){
        ...
        cout<<endl;
        cin>>d[i];
        if(d[i]>best){best=d[i]; s=i;}
    }
    vector<char> alive(n+1,1);
    int cur=n;
    vector<int> ord; ord.reserve(n);
    for(int i=1;i<=n;i++) if(i!=s) ord.pb(i);
    sort(all(ord), [&](int x,int y){ return d[x]>d[y]; });
    for(int x:ord){
        if(cur==best) break;
        cout<<"? "<<s<<" "<<cur-1;
        for(int y=1;y<=n;y++) if(alive[y] && y!=x) cout<<" "<<y;
        cout<<endl;
        int r; cin>>r;
        if(r==best){ alive[x]=0; --cur; }
    }
    vector<pair<int,int>> ans;
    for(int i=1;i<=n;i++) if(alive[i]) ans.pb({d[i],i});
    sort(ans.begin(), ans.end(), greater<pair<int,int>>());
    cout<<"! "<<sz(ans);
    for(auto &p:ans) cout<<" "<<p.se;
    cout<<endl;
}
